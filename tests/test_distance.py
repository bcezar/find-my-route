...
async def test_build_matrix_uses_osrm_when_available():
    ...
    fake_durations = [[0, 720,  480 ], [720,  0, 360 ], [480,  360,  0]]
    osrm_response = {"code": "Ok", "distances": fake_distances, "durations": fake_durations}
    ...
    # OSRM returns seconds → converted to minutes
    assert dur_matrix[0][1] == pytest.approx(12.0)
    assert dur_matrix[1][2] == pytest.approx(6.0)
    assert dur_matrix[0][0] == pytest.approx(0.0)
